...
exports.sendAcceptanceEmail = functions.firestore
    ...
    return admin
        ...
        const apiKey = functions.config().sendgrid.key;
        sgMail.setApiKey(apiKey);
        const html = AdmittedEmail_1.default(user.displayName);
        const msg = {
            to: user.email,
            from: "confirm@hacknyu.org",
...
exports.sendReminderEmail = () => {
    ...
    const admittedUsers = [];
    let emails;
    let html = ReminderEmail_1.default();
    return db
        .collection("admitted")
        .get()
        .then(snapshot => {
...
exports.getApplicationStats = functions.https.onCall(() => {
    const db = admin.firestore();
    let nyuCount = 0;
    let totalCount = 0;
...
exports.getRejectedUsersData = () => {
    ...
    return db
        ...
        return Promise.all(Object.keys(rejectedUsersData)
